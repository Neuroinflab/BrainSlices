...
#include "StreamedImageOutputWindow.h"
#include "StreamCommandlineParser.h"
...
StreamedImageOutputWindow<Precision> * outputBuffer = NULL;
StreamCommandlineParser * options = NULL;
...
void run()
{
  ...
  if (!initized)
  {
    ...
    if (!initized)
    {
      globalStep = options->cropWidth > options->threads ?
...
{
  options = new StreamCommandlineParser(argc, argv);
  ...
  if (options->dst != "-")
  {
    ...
    if (ofh == NULL)
    {
      tsErr.printf("ERROR: Unable open '%s' for output.\n", options->dst.c_str());
      return 2;
    }
  }
  ...
  // when threading used only for output
  if (options->maxval > 255)
  {
    shortRow = new unsigned short [options->cropWidth * d];
  }
  else
  {
    charRow = new unsigned char [options->cropWidth * d];
  }
  ...
  size_t bufferingInput = StreamedImagePrescaledInputWindow<Precision>::bufferSize(options->srcWidth,
                                                                                   ...
                                                                                   (void *) charRow,
                                                                                   0)
                          + StreamedImageOutputWindow<Precision>::bufferSize(options->cropWidth,
                                                                             options->cropHeight,
                                                                             0,
                                                                             options->channels,
                                                                             options->maxval,
                                                                             ...
                                                                             0,
                                                                             true);
  size_t bufferingOutput = StreamedImageOutputWindow<Precision>::bufferSize(options->cropWidth,
                                                                            options->cropHeight,
                                                                            radiusY,
                                                                            options->channels,
                                                                            options->maxval,
  ...
  else
  {
    bufferingInput = StreamedImagePrescaledInputWindow<Precision>::bufferSize(options->srcWidth,
                     ...
                     + StreamedImageOutputWindow<Precision>::bufferSize(options->cropWidth,
                                                                        ...
                                                                        0,
                                                                        false);
    bufferingOutput = StreamedImageOutputWindow<Precision>::bufferSize(options->cropWidth,
                                                                       options->cropHeight,
                                                                       radiusY,
                                                                       options->channels,
                                                                       options->maxval,
  ...
  size_t outputBufferLineCost = StreamedImageOutputWindow<Precision>::lineSize(options->cropWidth, d, normalized);
  if (bufferInputWindow)
  {
    ...
    outputBuffer = new StreamedImageOutputWindow<Precision>(options->cropWidth,
                                                            options->cropHeight,
                                                            0,
                                                            ofh,
                                                            options->channels,
