...
(function(BS, $, undefined)
{
  ...
  else
  {
    ...
    api.CSynchronizedStacksDisplay = function($display, nx, ny, synchronize,
                                              ...
                                              $controlPanel, gfx, images, toolbar)
    {
      ...
      else
      {
        ...
        this.updateZoom = function()
        {
          if (thisInstance.zoomUpdateEnabled)
          {
            ...
            thisInstance.$zoomBar.slider("value",zoomLog);
        ...
        this.updateZoomBar = function()
        {
          if (thisInstance.zoomUpdateEnabled)
          {
            thisInstance.zoomUpdateEnabled = false;
            var zoom = Math.pow(2, thisInstance.$zoomBar.slider("value"));
            var zoomLog = thisInstance.$zoomBar.slider("value");
            thisInstance.setZoom(zoom);
            thisInstance.update();
            if (thisInstance.$zoomLog.val() != zoomLog)
            {
        ...
        this.$zoomBar.on('slidechange', this.updateZoomBar);
        ...
        this.updateZoomLog = function()
        {
          if (thisInstance.zoomUpdateEnabled)
          {
            ...
            var zoomLog = thisInstance.$zoomLog.val();
            thisInstance.setZoom(zoom);
            thisInstance.update();
            if (thisInstance.$zoom.val() != zoom)
            {
    ...
    api.CSynchronizedStacksDisplay.prototype =
    {
      ...
      function()
      {
        if (this.zoomUpdateEnabled)
        {
          ...
          {
              this.$zoomBar.slider("value",zoom);
          }
        }
      },
